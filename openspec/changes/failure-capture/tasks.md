...
- [x] 8.10 GREEN `schemas.py`: `ResultListItemResponse` (nested `FailureProjection` shape); `ResultDetailResponse` (every field, field by field, never `from_attributes`).
- [x] 8.11 GREEN `routes/read.py`: `GET /api/v1/runs/{run_id}/result?node_id=` (`Query(..., max_length=MAX_IDENTITY_CHARS)`) calling `store.get_result`; `_result_item` reads from `ResultListEntry`/`FailureProjection`, never the full `Result`.
- [x] 8.12 GREEN `openapi/v1.yaml`: new `read`-tagged `GET /runs/{run_id}/result` operation, hand-written.
