...
- [ ] 3.1 RED: `test_routes_sections.py` — POST 201/200; trailing-slash coercion; empty/whitespace name rejected; `Unassigned`/`UNASSIGNED` rejected case-insensitively; over-length name/prefix rejected; `too_many_sections` at the bound; DELETE 204 then 404 `unknown_section`; GET lists an upserted section; a CR/LF + `</script>` name is rejected without appearing in the body; a quoting-shaped name round-trips byte-identically (spec test-sections: name constraints, trailing slash, listed section; design.md threat notes)
- [ ] 3.2 GREEN: six rejection classes in `service/errors.py` — `InvalidSectionNameError`, `ReservedSectionNameError`, `InvalidSectionPrefixError`, `UnknownSectionError`, `TooManySectionsError`, `UnreadableSettingError`; update `__all__` (D89)
- [ ] 3.3 GREEN: `SectionValue`, `SectionUpsertRequest`, `SectionResponse`, `SectionListResponse` in `service/schemas.py`
- [ ] 3.4 GREEN: create `service/routes/sections.py` — `TEST_SECTIONS_NAMESPACE` constant, definition loader, GET/POST/DELETE `/config/sections` (D87, D89); typed `store: ExecutionStore = request.app.state.store`
- [ ] 3.5 GREEN: `include_router(sections_router, prefix="/api/v1")` in `service/app.py`
- [ ] 3.6 GREEN: hand-write the three operations into `service/openapi/v1.yaml`, `read`/`write` tagged (D87)
- [ ] 3.7 GREEN: add the GET `read` path's binding-table entry in `test_read_only_surface.py`
- [ ] 3.8 Verify: 3.1 green; `test_interface_document.py` and `test_read_only_surface.py` still pass
